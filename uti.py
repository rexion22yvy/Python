...
def find_critical_servers(df):
    critical_servers = df[(df['Free memory'] / df['Total Memory'] < 0.1) & (df['Used Swap'] / df['Total Swap'] > 0.1)]
    return critical_servers[['server name', 'Free memory', 'Total Memory', 'Used Swap', 'Total Swap']]
